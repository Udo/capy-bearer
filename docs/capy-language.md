...
The current implementation includes the lexer, expression parser, bounded diagnostics, direct Wasm encoding, `.capy` artifact integration, and real Bearer CLI/HTTP execution. Scalar control flow plus ARC strings, arrays, and nominal structs execute as native Wasm. Parameter `any` now monomorphizes lazily by concrete argument types, caches specializations, validates operators after substitution, supports `x::type` results, prefers exact overloads, and rejects equally ranked generic matches. Explicit `as` conversions, tuples, reflection, weak references, closures/function values, DValue adapters, and Capy/C++ calls remain open. The authoritative remaining work is tracked in `/root/docs/work/capy-compiler.md`.
