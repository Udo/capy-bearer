...
- [x] The formatter review preserves output and avoids a slower interpolation path.
...
- [x] G7: Evaluate Capy formatter replacement. Retain native format helpers when the replacement changes output or slows conversion.
...
- 2026-08-28: Tested a Capy f64 formatter with `bits_of()` and `string(dval(value))`. It changed infinity and NaN output and added DValue allocation and host imports. The prototype unit grew from 4,850 to 6,274 bytes. Retained the native formatter to preserve output and the interpolation hot path.
- 2026-08-28: build_linux, service restart, native, Phase 1, reference, guide, documentation, compile-timeout, CLI, golden write, and golden check all passed.
