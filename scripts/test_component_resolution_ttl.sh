...
printf '%s\n' 'function COMPONENT(request : dval) { print("b") }' >"$source_dir/b.capy"
ln -s a.capy "$source_dir/chosen.capy"

declare -A seen
collect_workers() {
	local method="$1"
	local marker="$2"
	local path="${3:-parent.capy}"
...
collect_missing_workers() {
	...
	while (( SECONDS < deadline && ${#seen[@]} < worker_count )); do
		local response status body pid
		response=$(curl -sS --max-time 10 -H "Host: $http_host" -w $'\n%{http_code}' "http://127.0.0.1/$test_name/missing.capy")
		status=${response##*$'\n'}
		body=${response%$'\n'*}
		if [[ "$status" == "503" ]]; then
			sleep 0.1
...
sleep 10.2
collect_workers GET a
ln -sfn b.capy "$source_dir/chosen.capy"
collect_workers GET a
...
EOF
collect_missing_workers
printf '%s\n' 'function COMPONENT(request : dval) { print("created") }' >"$source_dir/later.capy"
collect_workers GET created missing.capy
