...
source_file="$root/site/entry.capy"
...
values = {"BIN_DIRECTORY": f"{root}/bin", "TMP_UPLOAD_PATH": f"{root}/uploads", "SESSION_PATH": f"{root}/sessions", "TASK_DIRECTORY": f"{root}/tasks", "SITE_DIRECTORY": f"{root}/site", "FCGI_SOCKET_PATH": f"{root}/run/fastcgi.sock", "FCGI_PORT": "", "HTTP_SOCKET_PATH": f"{root}/run/http.sock", "HTTP_PORT": "", "CLI_SOCKET_PATH": f"{root}/run/cli.sock", "WS_BROKER_SOCKET_PATH": f"{root}/run/ws.sock", "WORKER_COUNT": "1", "PROACTIVE_COMPILE_ENABLED": "0", "TASK_WORKERS": "1"}
...
printf '%s\n' 'function CLI(request : dval) { print("abi-pin-ok") }' >"$source_file"
...
[[ "$(scripts/bearer-cli /entry.capy)" == abi-pin-ok ]]
