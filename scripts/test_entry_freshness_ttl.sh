...
cat >"$source_dir/child.capy" <<'EOF'
#exports Marker
struct Marker { value : string }
function COMPONENT(request : dval) { print("marker-a") }
EOF
...
http_probe() {
	curl -fsS --max-time 5 -H "Host: $http_host" "http://127.0.0.1/$test_name/parent.capy"
}
...
parent_wasm="$cache_dir/parent.capy.wasm"
exec 8>"$parent_wasm.lock"
flock 8
sed -i 's/marker-a/marker-b/' "$source_dir/child.capy"
rm -f "$mutation_file"
post_status=$(curl -sS --max-time 5 -o "$post_body" -D "$post_headers" -w '%{http_code}' -X POST -H "Host: $http_host" "http://127.0.0.1/$test_name/parent.capy")
if [[ "$post_status" != "503" || -e "$mutation_file" ]] || ! grep -qi '^Retry-After: 1' "$post_headers"; then
	echo "stale mutation did not fail closed: status=$post_status body=$(<"$post_body")" >&2
	exit 1
fi
...
collect_all_workers 1 marker-b
touch "$cache_dir/parent.capy.meta.txt"
...
cli_output=$(timeout --signal=TERM --kill-after=3s 20s scripts/bearer-cli "/$test_name/parent.capy")
