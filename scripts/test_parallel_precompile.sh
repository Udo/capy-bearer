...
precompile() {
	timeout "$precompile_timeout" env BEARER_PRECOMPILE_FILES_IN="$source_dir" BEARER_PRECOMPILE_BIN_DIRECTORY="$bin_directory" "$@" bin/bearer_fastcgi.linux.bin --precompile
}
...
precompile BEARER_PRECOMPILE_JOBS=1 BEARER_WASM_PCH_DIR="$artifact_dir/pch-shared" >/dev/null
rm "$source_dir/warmup.uce"

write_units serial
serial_start=$(date +%s%N)
serial_output=$(precompile BEARER_PRECOMPILE_JOBS=1 BEARER_WASM_PCH_DIR="$artifact_dir/pch-shared")
...
parallel_output=$(precompile BEARER_PRECOMPILE_JOBS=2 BEARER_WASM_PCH_DIR="$artifact_dir/pch-shared")
...
race_output=$(precompile BEARER_PRECOMPILE_JOBS=2 BEARER_WASM_PCH_DIR="$artifact_dir/pch-race")
...
precompile BEARER_PRECOMPILE_JOBS=2 BEARER_WASM_PCH_DIR="$artifact_dir/pch-shared" >"$generation_log" 2>&1 &
generation_pid=$!
...
generation_retry_output=$(precompile BEARER_PRECOMPILE_JOBS=2 BEARER_WASM_PCH_DIR="$artifact_dir/pch-shared")
...
set +e
failure_output=$(precompile BEARER_PRECOMPILE_JOBS=2 BEARER_WASM_PCH_DIR="$artifact_dir/pch-shared" 2>&1)
failure_rc=$?
set -e
...
set +e
repeat_failure_output=$(precompile BEARER_PRECOMPILE_JOBS=2 BEARER_WASM_PCH_DIR="$artifact_dir/pch-shared" 2>&1)
repeat_failure_rc=$?
set -e
