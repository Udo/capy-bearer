...
function RENDER(request : dval) {
    ...
    var clean := mysql_connected(db) && mysql_connected(other) && mysql_connected(unselected) && string(primary[0]["db"], "") == "$test_database" && string(primary[0]["label"], "") == "primary" && string(secondary[0]["db"], "") == "$test_database_other" && string(secondary[0]["label"], "") == "other" && string(no_database[0]["db"], "") == "" && string(marker[0]["marker"], "") == "" && mysql_error(db) != ""
...
function RENDER(request : dval) {
    ...
    if !mysql_connected(db) && mysql_error(db) != "" { print("database-selection-failed") }
